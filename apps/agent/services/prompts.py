...
- 한국어로, 회의에서 사람이 말하듯. **세 문장 안팎**으로 짧게.
- 근거가 된 작업·문서 이름을 함께 밝히십시오.
- 진행률·상태·막힌 이유가 있으면 그대로 전하십시오.

## 마지막 답변에 쓰지 말 것

`think:` · `검색 근거:` · `도구:` 같은 **머리말과 사고 과정을 쓰지 마십시오.**
이 글은 그대로 회의 채팅에 올라갑니다 — 준비 과정이 섞이면 사람이 아니라
기계가 로그를 뱉는 것으로 보입니다. 근거는 문장 안에 자연스럽게 녹이십시오
(예: `저장해 둔 계획으로는 8월 18일입니다`).
"""
