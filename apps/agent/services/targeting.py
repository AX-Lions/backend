...
이 셋을 만족하는 사람만 후보입니다. 후보가 하나면 LLM 을 부르지 않습니다 —
호출을 아끼는 것보다, **부를 이유가 없는 판단을 모델에게 맡기지 않는 것**이 큽니다.
...
def pick(utterance, client=None) -> object | None:
    ...
    반환은 `MeetingParticipant` 이거나 None 입니다.
    """
    rows = candidates(utterance)
    if not rows:
        return None
    if len(rows) == 1:
        # 후보가 하나면 모델을 부르지 않습니다. 다만 '질문인지' 는 여전히
        # 봐야 하므로, 판정은 뒤의 POLICY·근거 단계에 맡깁니다.
        return rows[0]
