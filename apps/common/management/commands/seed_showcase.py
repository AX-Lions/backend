...
class Command(BaseCommand):
    ...
    def _report(self, team, project, people):
        ...
        self.stdout.write(
            ...
            f"\n  촬영 순서\n"
            f"    1. {lead.name} 계정으로 로그인 → 회의 `API 명세 리뷰` 를 만든다\n"
            f"    2. {away.name} 계정으로 로그인 → 불참 등록 → 회의 전 준비\n"
            f"       (논쟁점 예측 · 입장 등록 · 회의별 대리인 설정)\n"
            f"    3. Discord 에서 `/meeting-start` 로 그 회의에 스레드를 붙이고 진행\n"
            f"       - 일정 연장 얘기를 꺼내면 {away.name}의 Bordo 가 **유보**합니다\n"
            f"         (`allow_schedule_change=False` 로 깔아 뒀습니다)\n"
            f"    4. 회의 종료 → {away.name} 계정으로 로그인 → 브리핑 · 답변 대기 질문 처리\n")
