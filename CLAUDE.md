...
| `delegate` 에 `allowed_categories` · `capabilities` · `speak_mode` | `enabled` · `sources` · `prompt` 뿐 | 불참 팝업에 대응하는 입력이 없습니다. `sources` 4종(`work`·`plan`·`thought`·`document`)이 화면의 체크박스와 1:1 입니다 |
| 대리인 방 제목이 저장값 | 조회 시점에 주인의 호칭으로 조립 | 개인 설정에서 이름을 바꿨는데 방 제목만 옛 이름이면 저장이 안 된 줄 압니다. 어차피 이름을 못 바꾸는 방입니다 |
| `AgentSettings` 저장 = 버전 증가 | `tone` · `agent_name` 은 **버전을 안 올림** | 말투·호칭은 판정이 아니라 표현입니다. 올리면 아무 기준도 안 바뀐 버전이 판정 이력에 끼어듭니다 |
| `indexes` 가 회의 스코프 | `category=WORK` 은 **프로젝트 + 기간** | 작업 엣지에는 회의가 없어 회의로 좁히면 언제나 빈 배열입니다 |
